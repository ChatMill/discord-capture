...
def test_build_discord_embeds_from_supplement_request(monkeypatch):
    ...
    req = SupplementRequest(session_id="sid", event_id="eid", operator_id="uid", payload=task, history=[], question="cont")
    # dict 化
    req_dict = req.model_dump()
    req_dict["event_type"] = "supplement_request"
    req_dict["task"] = task.model_dump()
    ...
    assert embeds[1].title == "🗂️ Task Info"
