...
class DummyEvent(Event):
    ...
    def __init__(self, event_id):
        super().__init__(event_type="capture", session_id="sid", event_id=event_id, operator_id="op", payload=Task(chatmill_id="cmid", message_ids=["m1"], title="t", description="d"), history=[])
...
def test_build_session():
    ...
    task = Task(chatmill_id="cmid", message_ids=["m1"], title="t", description="d")
    event = DummyEvent(event_id="eid")
    session_id = "sid"
    session = session_schema.build_session(source, task, event, session_id)
    # 检查 Session 字段
    assert session.session_id == session_id
    assert session.source == source
    assert session.payload == task
